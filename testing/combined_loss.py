...
if __name__ == "__main__":
    ...
    def loss2(y_pred):
        return tf.ones_like(y_pred) + 1e-5 * y_pred

    loss_fn = CombinedLoss(loss1, loss2, num_epochs)

    # Example usage during model.fit
    model.compile(optimizer="adam", loss=loss_fn)

    model.fit(x_train, y_train, epochs=num_epochs, callbacks=[loss_fn.callback])
